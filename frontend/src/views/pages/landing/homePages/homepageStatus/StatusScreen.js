...
import image1 from "../../../../../assets/images/landing/homePageStatus/mobille.avif";
import image2 from "../../../../../assets/images/landing/homePageStatus/status_2.webp";
import image3 from "../../../../../assets/images/landing/homePageStatus/status_3.webp";
import image4 from "../../../../../assets/images/landing/homePageStatus/status_4.webp";
import image5 from "../../../../../assets/images/landing/homePageStatus/status_5.webp";
...
const statusesData = [
  {
    imageSrc: [
      { media: image1, type: "image" },
      { media: image3, type: "image" },
    ],
  ...
  {
    imageSrc: [
      { media: image2, type: "image" },
      { media: image1, type: "image" },
      { media: image3, type: "image" },
      { media: image4, type: "image" },
      { media: image5, type: "image" },
    ],
    title: "thisisthenickname123",
    description: "Kazi_Steff thirty lettersetsss",
    text: "I will make electrician 2d electric service explainer video",
...
const generateInitialStatus = (length) => {
  const initialStatus = [];
...
const StatusScreen = () => {
  ...
  const [currentStatusScreen, setCurrentStatusScreen] = useState(
    generateInitialStatus(initialLength)
  );
  ...
  useEffect(() => {
    if (!isPaused && current < duration) {
      ...
      setCurrentStatusScreen((prevStatusScreen) => {
        if (
          prevStatusScreen[activeIndex].statusNo ===
          statusesData[activeIndex].imageSrc.length - 1
        ) {
          ...
          newArray[activeIndex].animation[newArray[activeIndex].statusNo] =
            newArray[activeIndex].statusNo;
          return newArray;
        }
      });
    }
  ...
  const handleActiveCurrentStatus = (swiper, index) => {
    ...
    if (
      clickX > middlePoint &&
      currentStatusScreen[index].statusNo ==
        statusesData[index].imageSrc.length - 1
    ) {
      if (swiperRef.current.isEnd) {
        dispatch(showGameStatus(false));
      } else {
      ...
      clickX > middlePoint &&
      currentStatusScreen[index].statusNo < statusesData[index].imageSrc.length
    ) {
      setCurrentStatusScreen((prevArray) => {
        const newArray = [...prevArray];
        newArray[index].statusNo = newArray[index].statusNo + 1;
        newArray[index].animation[newArray[index].statusNo] =
          newArray[index].statusNo;
        return newArray;
      });
    } else if (
      clickX < middlePoint &&
      currentStatusScreen[index].statusNo == 0
    ) {
              ...
              <SwiperSlide key={index}>
                ...
                >
                      ...
                      {activeIndex == index &&
                        statusData.imageSrc.map((cur, i) => (
                          <Grid
                            key={i}
                            item
                            xs={12 / statusData.imageSrc.length}
                            sx={{ px: "3px !important" }}
                          >
                            <Grid
                              sx={{
                  ...
                  >
                    <img
                      src={
                        statusData.imageSrc[currentStatusScreen[index].statusNo]
                          .media
                      }
                      alt="story"
                      style={{
